...
var time = require('ddv-auth/util/time.js')
var url = require('ddv-auth/util/url')
var sign = require('ddv-auth/util/sign')
var parseDataByBody = require('ddv-restful-api/lib/parseDataByBody.js')
// 请求模块
...
Object.assign(session, {
  getTrueData: function getTrueData (sessionInitUrl, tryNum, api) {
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      isRunIng || getTrueDataRun(sid, sessionInitUrl, queue, tryNum, api)
...
function sessionInit (sessionInitUrl, data, api) {
  ...
  var urlObj = url.parse(sessionInitUrl)
  // 返回一个请求
  return request(sessionInitUrl, '', 'GET', {
    Authorization: authorization,
    Host: urlObj.host + ':' + urlObj.port
  }, api)
    .then(function (res) {
...
function getTrueDataRun (sid, sessionInitUrl, queue, tryNum, api) {
  ...
  return (
    ...
    .then(function (data) {
      if (tryNum !== true && data && data.sessionId && data.sessionKey && data.sessionCard && data.expiresTime && util.time() < (data.expiresTime - 5) && parseInt(tryNum) < 1) {
        ...
        return session.init(sessionInitUrl, data, api)
          .then(function (data) {
...
function session (sessionInitUrl, tryNum, api) {
  return session.getTrueData(sessionInitUrl, tryNum, api)
}
